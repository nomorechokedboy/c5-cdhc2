...
function RouteComponent() {
  // ...existing code...
  const [studentParams, setStudentParams] = React.useState<StudentQueryParams>({ politicalOrg: 'hcyu' });
  const { data: battalions = [], isLoading: isLoadingUnits } = useUnitsData({ level: 'battalion' });
  const [selectedBattalionId, setSelectedBattalionId] = React.useState<number | null>(null);
  ...
  const selectedBattalion = battalions.find((b) => b.id === selectedBattalionId) || null;
	  ...
	  <SidebarInset>
		<div className="hidden h-full flex-1 flex-col space-y-8 p-8 md:flex">
		  <div className="flex items-center justify-between space-y-2">
			<div>
			  <h2 className="text-2xl font-bold tracking-tight">Danh sách học viên là đoàn viên</h2>
			  <p className="text-muted-foreground">Chọn tiểu đoàn, đại đội, lớp để xem bảng học viên</p>
			</div>
		  </div>
