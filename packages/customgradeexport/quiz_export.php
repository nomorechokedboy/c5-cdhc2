...
/**
 * Quiz grade export script with template selection
 *
...
if ($action === 'export') {
    ...
    if ($templateid) {
        $templatePath = \local_customgradeexport\template_manager::get_template_path('quiz', $templateid);
        $format = \local_customgradeexport\template_manager::get_template_format('quiz', $templateid);
...
$templates = \local_customgradeexport\template_manager::get_templates('quiz');
...
$defaulturl = new moodle_url('/local/customgradeexport/quiz_export.php', [
    'cmid' => $cmid,
    'action' => 'export'
]);
...
if (!empty($templates)) {
    ...
    foreach ($templates as $id => $template) {
        ...
        $exporturl = new moodle_url('/local/customgradeexport/quiz_export.php', [
            'cmid' => $cmid,
            'action' => 'export',
            'template' => $id
        ]);
